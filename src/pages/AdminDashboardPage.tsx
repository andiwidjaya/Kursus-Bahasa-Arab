...
import { Course, Module, Lesson, CourseLevel, CourseCategory, PaymentStatus, VocabItem } from '../types';
import { 
...
export const AdminDashboardPage: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'metrics' | 'courses' | 'modules' | 'lessons' | 'orders' | 'users' | 'vocab'>('metrics');
  ...
  const [vocabCourseFilter, setVocabCourseFilter] = useState<string>('ALL');
  ...
  const handleSaveVocab = (e: React.FormEvent) => {
    e.preventDefault();
    if (!vocabForm.arabic || !vocabForm.transliteration || !vocabForm.indo_meaning) {
      showToast('Harap isi Teks Arab, Transliterasi, dan Arti Bahasa Indonesia.', 'error');
      return;
    }
    if (editingVocab) {
      updateVocabItem(editingVocab.id, vocabForm);
    } else {
      createVocabItem(vocabForm);
    }
  ...
  const handleEditVocab = (item: VocabItem) => {
    setEditingVocab(item);
    setVocabForm({
      arabic: item.arabic,
      transliteration: item.transliteration,
      indo_meaning: item.indo_meaning,
  ...
  const handleDeleteVocab = (id: string, text: string) => {
    if (window.confirm(`Apakah Anda yakin ingin menghapus kosakata "${text}"?`)) {
      deleteVocabItem(id);
    }
  };
  ...
  const filteredVocabs = vocabItems.filter(v => {
    const matchQuery = 
      v.arabic.includes(vocabSearch) ||
      v.transliteration.toLowerCase().includes(vocabSearch.toLowerCase()) ||
      v.indo_meaning.toLowerCase().includes(vocabSearch.toLowerCase());
    const matchCat = vocabCategoryFilter === 'ALL' || v.category.toLowerCase().includes(vocabCategoryFilter.toLowerCase());
    const matchCourse = vocabCourseFilter === 'ALL' || v.course_id === vocabCourseFilter;
    return matchQuery && matchCat && matchCourse;
  });
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div className="flex border-b border-slate-200 bg-white rounded-2xl p-2 shadow-sm overflow-x-auto">
        {[
          ...
          { id: 'vocab', label: `📖 Kosakata (${vocabItems.length})`, icon: BookOpen },
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-5 rounded-3xl border border-slate-200 shadow-sm">
            <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
              <div className="relative flex-1 sm:w-64">
                ...
                <input
                  type="text"
                  placeholder="Cari Arab, transliterasi, arti..."
                  value={vocabSearch}
              ...
              <select
                ...
                onChange={e => setVocabCategoryFilter(e.target.value)}
                className="px-3 py-2.5 rounded-xl border border-slate-200 text-xs bg-white focus:ring-2 focus:ring-emerald-500 outline-none font-medium"
              >
                <option value="ALL">Semua Kategori</option>
                <option value="Ism">Ism (اسم)</option>
                <option value="Fi'il">Fi'il (فعل)</option>
                <option value="Harf">Harf (حرف)</option>
              </select>

              <select
                value={vocabCourseFilter}
                onChange={e => setVocabCourseFilter(e.target.value)}
                className="px-3 py-2.5 rounded-xl border border-slate-200 text-xs bg-white focus:ring-2 focus:ring-emerald-500 outline-none font-medium max-w-[200px] truncate"
              >
            ...
            <button
              ...
              className="w-full sm:w-auto px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-colors shadow-md flex items-center justify-center gap-2 shrink-0"
            >
              <Plus className="w-4 h-4" /> Tambah Kosakata Baru
            </button>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left text-xs">
                <thead className="bg-slate-50 text-slate-500 uppercase tracking-wider font-bold border-b border-slate-200">
                  <tr>
                    <th className="p-3.5">Kata (Arab)</th>
                    <th className="p-3.5">Transliteration & Arti</th>
                    <th className="p-3.5">Kategori</th>
                    <th className="p-3.5">Contoh Kalimat</th>
                    <th className="p-3.5">Kursus Terkait</th>
                  ...
                  {filteredVocabs.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="p-8 text-center text-slate-400">
                        Tidak ada kosakata yang cocok dengan pencarian / filter.
                      </td>
                    </tr>
                  ) : (
                      ...
                      return (
                        <tr key={item.id} className="hover:bg-slate-50/60 transition-colors">
                          <td className="p-3.5 font-bold">
                            <span className="text-xl text-emerald-700 font-serif leading-relaxed" dir="rtl">
                              {item.arabic}
                            </span>
                          </td>
                          <td className="p-3.5">
                            <p className="font-bold text-slate-900">{item.indo_meaning}</p>
                            <p className="text-[11px] text-slate-500 italic">{item.transliteration}</p>
                          </td>
                          <td className="p-3.5">
                          ...
                          <td className="p-3.5 text-slate-600 max-w-xs truncate">
                            {item.example_sentence ? (
                              <span dir="rtl" className="font-serif text-slate-800 text-sm">{item.example_sentence}</span>
                            ) : (
                          ...
                          <td className="p-3.5 text-slate-600">
                            <span className="px-2 py-0.5 rounded text-[10px] font-medium bg-slate-100 text-slate-700">
                              {linkedCourse ? linkedCourse.title : 'Umum'}
                            </span>
                          </td>
                          <td className="p-3.5 text-right space-x-1 shrink-0">
                            <button
                              onClick={() => handleEditVocab(item)}
                            ...
                            <button
                              onClick={() => handleDeleteVocab(item.id, item.arabic)}
      ...
      {/* MODAL: CREATE / EDIT VOCAB */}
      {isVocabModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl p-6 border border-slate-100 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b border-slate-100 pb-3">
              <h3 className="font-black text-slate-900 text-base">
                {editingVocab ? 'Edit Kosakata Bahasa Arab' : 'Tambah Kosakata Baru'}
              </h3>
              <button
            ...
            <form onSubmit={handleSaveVocab} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Teks Bahasa Arab (باَلْحَرَكَاتِ) <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  placeholder="e.g. كِتَابٌ"
                  value={vocabForm.arabic}
                  onChange={e => setVocabForm({ ...vocabForm, arabic: e.target.value })}
                  dir="rtl"
                  className="w-full p-3 rounded-xl border border-slate-200 text-lg font-serif text-slate-900 focus:ring-2 focus:ring-emerald-500 outline-none"
                  required
                />
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Transliterasi (Cara Baca) <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  placeholder="e.g. Kitaabun"
                  value={vocabForm.transliteration}
                  onChange={e => setVocabForm({ ...vocabForm, transliteration: e.target.value })}
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1">
                  Arti Bahasa Indonesia <span className="text-rose-500">*</span>
                </label>
                <input
                  type="text"
                  placeholder="e.g. Buku / Kitab"
                  value={vocabForm.indo_meaning}
                  onChange={e => setVocabForm({ ...vocabForm, indo_meaning: e.target.value })}
                  className="w-full p-2.5 rounded-xl border border-slate-200 text-xs focus:ring-2 focus:ring-emerald-500 outline-none"
                  required
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  >
                    <option value="Ism">Ism (اسم / Kata Benda)</option>
                    <option value="Fi'il">Fi'il (فعل / Kata Kerja)</option>
                    <option value="Harf">Harf (حرف / Kata Sambung)</option>
                    <option value="Ungkapan">Ungkapan (تَعْبِيْرٌ)</option>
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    value={vocabForm.course_id}
                    onChange={e => setVocabForm({ ...vocabForm, course_id: e.target.value })}
                    className="w-full p-2.5 rounded-xl border border-slate-200 text-xs bg-white focus:ring-2 focus:ring-emerald-500 outline-none"
                  >
                    {courses.map(c => (
                      <option key={c.id} value={c.id}>{c.title}</option>
              ...
              <div>
                ...
                <textarea
                  placeholder="e.g. هَذَا كِتَابٌ جَدِيْدٌ"
                  value={vocabForm.example_sentence}
                  onChange={e => setVocabForm({ ...vocabForm, example_sentence: e.target.value })}
                  dir="rtl"
                  rows={2}
                  className="w-full p-2.5 rounded-xl border border-slate-200 text-base font-serif text-slate-900 focus:ring-2 focus:ring-emerald-500 outline-none"
                />
              </div>
              ...
              <div className="flex justify-end gap-2 pt-2">
                ...
                <button
                  type="submit"
                  className="px-5 py-2.5 rounded-xl text-xs font-bold text-white bg-emerald-600 hover:bg-emerald-700 transition-colors shadow-md"
                >
                  {editingVocab ? 'Simpan Perubahan' : 'Tambah Kosakata'}
                </button>
              </div>
            </form>
          </div>
